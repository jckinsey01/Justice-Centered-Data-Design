...
```js
let dateParser = utcParse("%d/%m/%Y")
let ballotsWithDateObjs = nc2024SampleVoters.map(
  (ballot) => {
    ballot.ballot_re_dt_obj = dateParser(ballot.ballot_req_dt)
return ballot
}
)
```
...
<!-- E2 -->
```js
const formatDate = d3.utcFormat("%a, %B, %e, %Y");
const ballotsWithDateObjects = [
  { id: 1, ballot_req_dt_obj: new Date(Date.UTC(1981, 0, 27))},
  {id: 2, ballot_req_dt_obj: new Date(Date.UTC(2025, 8, 12))},
  {id: 3, ballot_req_dt_obj: new Date(Date.UTC(2024, 11, 25))},
];
const updatedBallots = ballotsWithDateObjs.map(d => {
  return{
    ...d,
    ballot_req_dt: formatDate(d.ballot_req_dt_obj)
